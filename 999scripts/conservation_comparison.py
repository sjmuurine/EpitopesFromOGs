...
def calculate_conservation(sequences, gap_threshold=0.5):
    ...
    taxa_list = [s['taxon'] for s in sequences.values()]
    ...
    for pos in range(alignment_length):
        ...
        column_taxa = [taxa_list[i] for i, seq in enumerate(seq_list) if pos < len(seq)]
        ...
        # Get taxa that have the most common amino acid
        taxa_with_consensus = set(
            column_taxa[i] for i, aa in enumerate(column) 
            if aa == most_common_aa
        )
        taxa_per_position.append(taxa_with_consensus)
        
        # Conservation relative to all sequences
        conservation = most_common_count / num_sequences
        conservation_scores.append(conservation)
    
    return conservation_scores, consensus, taxa_per_position

def find_conserved_regions(conservation_scores, consensus, taxa_per_position, 
        ...
        if score >= threshold:
            ...
            if in_region:
                ...
                if length >= min_length:
                    # Get taxa present in this region
                    region_taxa = set.intersection(*taxa_per_position[start:i]) if taxa_per_position[start:i] else set()
    ...
    if in_region:
        ...
        if length >= min_length:
            region_taxa = set.intersection(*taxa_per_position[start:]) if taxa_per_position[start:] else set()
...
def analyze_single_orthogroup(aligned_file, threshold=0.6, min_length=10, gap_threshold=0.5):
    """Analyze conservation for a single orthogroup with taxon tracking"""
    ...
    conservation_scores, consensus, taxa_per_position = calculate_conservation(
        sequences, gap_threshold
    )
    ...
    regions = find_conserved_regions(conservation_scores, consensus, taxa_per_position, 
...
def batch_analyze(alignments_dir='alignments', output_dir='conservation_results',
                 threshold=0.6, min_length=10, gap_threshold=0.5):
    ...
    for aligned_file in aligned_files:
        ...
        result = analyze_single_orthogroup(aligned_file, threshold, min_length, gap_threshold)
...
if __name__ == "__main__":
    ...
    batch_analyze(args.alignments_dir, args.output_dir, 
                 args.threshold, args.min_length, args.gap_threshold)